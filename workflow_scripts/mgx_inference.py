...
def load_mgx_model(path, fp16, target, shapes={}):
    ...
    if os.path.isfile(f"{file}.onnx"):
        ...
        prog.compile(mgx.get_target(target))
    else:
...
class MGXSession():
    ...
    def __init__(self, path, fp16, target):
        self.path = path
        self.fp16 = fp16
        self.target = target
        self.program = load_mgx_model(path, fp16, target)
    ...
    def run(self, inputs):
        ...
        if (actual != expected):
            ...
            self.program = load_mgx_model(self.path, self.fp16, self.target, dict(actual))
        print("Running migraphx inference")
        return self.program.run(inputs)

def session(path, fp16, target):
    return MGXSession(path, fp16, target)
